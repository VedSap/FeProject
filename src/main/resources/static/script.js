...
function initMap() {
  map = new google.maps.Map(document.getElementById("map"), {
    zoom: 15,
    center: { lat: 18.623654, lng: 73.815518},
    mapTypeId: "satellite",
  });
...
async function getData(){
    const response = await fetch('http://localhost:8080/getlatlong')
...
function getPoints() {
    var response = getData().then(function toArray(response){
        var arr = new Array();
            for(let i=0; i<response.length; i++){
                console.log(typeof response[i].latitude);
                arr.push(new google.maps.LatLng(response[i].latitude, response[i].longitude));
            }
        return arr;
    });
